...
> 이메일 인증을 통해 받은 토큰을 사용하여 계정을 삭제합니다. 삭제된 계정 정보는 별도의 테이블에 보관됩니다.
...
| `deletionReason` | `string` | ✅   | 계정 삭제 사유                                            |
...
```json
{
  ...
  "deletionReason": "더 이상 서비스를 사용하지 않습니다"
}
```
...
| `401 Unauthorized`          | `WRONG_TOKEN_PURPOSE` | 토큰 목적이 일치하지 않습니다.          | `purpose`가 `DELETE_ACCOUNT`가 아닌 경우 |
